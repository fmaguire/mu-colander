...
void processVideo(char* videoFilename) {
    ...
    cv::Mat fg_mask; //fg mask fg mask generated by GMM method
    cv::Mat fg_mask2; //fg mask fg mask generated by GMM2 method
    cv::Mat fg_img;
    cv::Mat fg_img2; 
    //cv::Mat bg_img;

    cv::Ptr<cv::BackgroundSubtractor> pKNN = cv::createBackgroundSubtractorKNN();
    cv::Ptr<cv::BackgroundSubtractor> pGMM2 = cv::createBackgroundSubtractorMOG2();
    //improved adaptive background mixture model from [Zivkovic2004] and [Zivkovic2006]
    ...
    while( (char)keyboard != 'q' && (char)keyboard != 27 ){
        ...
        pGMM2->apply(frame, fg_mask2);

        cv::GaussianBlur(fg_mask, fg_mask, cv::Size(15, 15), 3, 3);
        cv::GaussianBlur(fg_mask2, fg_mask2, cv::Size(15, 15), 3, 3);
        //cv::threshold(fg_mask, fg_mask, 10, 255, 0);
        
        fg_img = cv::Scalar::all(0);
        frame.copyTo(fg_img, fg_mask);

        fg_img2 = cv::Scalar::all(0);
        frame.copyTo(fg_img2, fg_mask2);

        //pGMM2->getBackgroundImage(bg_img);
